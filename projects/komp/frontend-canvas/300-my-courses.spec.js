...
test.describe('My Courses | Canvas', () => {
  ...
  test('invite card shows accept and decline buttons', async ({ page }) => {
    ...
    await inviteItem.locator('button:has-text("Godta invitasjonen")').waitFor({ state: 'visible' });
    await inviteItem.locator('button:has-text("Avslå invitasjonen")').waitFor({ state: 'visible' });
  });
