...
test.describe("Frontpage | Canvas", () => {
  ...
  test("unenrolled card read-more shows module list", async ({ page }) => {
    ...
    await loginWithBasicAuth(page);
    await routeToFrontpage(page, true);
    // Wait for Vue card components to bind event handlers — the button is in
    // the DOM immediately but the click handler attaches after async init.
    await page.waitForLoadState("networkidle");

    const cardInstances = page
      .locator(".landing-page .card-container")
      .locator(
        `.card-instance h2:has-text("Programmering og algoritmisk tenkning")`,
      )
      .locator('xpath=ancestor::div[contains(@class, "card-instance")]');

    const readMoreButton = cardInstances
      .first()
      .locator('button.btn:has-text("Les mer")');
  ...
  test("unenrolled card enroll and unenroll", async ({ page }, testInfo) => {
    ...
    const cardInstance = page
      ...
      .locator(
        `.card-instance .card-box-title h2:has-text("Trygt og godt skolemiljø")`,
      )
      .locator('xpath=ancestor::div[contains(@class, "card-instance")]');
    ...
    if (isAlreadyEnrolled >= 1) {
      ...
      await page.waitForURL(/\/courses\//, { timeout: 30_000 });
      await page
        .locator('#actions_self_unenrollment:has-text("Meld deg av")')
        .click();
      const modalPopup = page.locator(".modal-box");
      await modalPopup.waitFor({ state: "visible" });
    ...
    await page
      ...
      .click();
    const goToCourse = page.locator(
      '.ic-Self-enrollment-footer a:has-text("Gå til emnet")',
    );
    const enrolled = await goToCourse
      .waitFor({ state: "visible", timeout: 5_000 })
      .then(
        () => true,
        () => false,
  ...
  test("invited card has no enroll or read-more buttons", async ({ page }) => {
    ...
    await cardInstance
      .first()
      .locator('button.btn:has-text("Meld deg på")')
      .waitFor({ state: "hidden" });
    await cardInstance
      .first()
      .locator('button.btn:has-text("Les mer")')
