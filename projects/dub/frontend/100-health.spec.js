...
test.beforeEach(({ browserName }) => {
  test.skip(
    ...
    "HTTP/DOM health checks: Chromium only",
  );
});
...
test.describe("Infrastructure", () => {
  test("unknown URL returns HTTP 404", async ({ page }) => {
    const response = await page.goto(
      "/this-page-does-not-exist-playwright-e2e-check",
    );
    expect(
      response.status(),
...
test.describe("SEO", () => {
  test("homepage has a meta description", async ({ page }) => {
    ...
    expect(
      content,
      "homepage meta description is missing or empty",
    ).toBeTruthy();
  });
  ...
  test("homepage declares hreflang for nynorsk (nn) variant", async ({
    page,
  }) => {
    test.skip(
      disabled.includes("hreflang"),
      "hreflang disabled for this environment in config.json",
    );
    await page.goto("/");
    ...
    await expect(
      nnLink,
      'hreflang="nn" missing — nynorsk variant not declared to search engines',
    ).toHaveCount(1);
  });
});
...
test.describe("Legal", () => {
  ...
  test("footer has tilgjengelighetserklæring (universal design declaration) link", async ({
    page,
  }) => {
    await page.goto("/");
