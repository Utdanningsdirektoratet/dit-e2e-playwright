...
test.describe("Navigation", () => {
  ...
  test("hamburger menu opens, navigates, and produces no console errors", async ({
    page,
  }) => {
    ...
    expect(
      newErrors,
      `Console errors after menu open/navigate:\n${newErrors.join("\n")}`,
    ).toEqual([]);
    expect(
      newWarnings,
      `Console warnings after menu open/navigate:\n${newWarnings.join("\n")}`,
    ).toEqual([]);
  });

  test("hamburger menu toggle (open + close) produces no console errors", async ({
    page,
  }) => {
    const checker = createConsoleChecker(page, CONSOLE_WHITELIST);
    await page.goto("/");
    ...
    expect(
      newErrors,
      `Console errors after menu toggle:\n${newErrors.join("\n")}`,
    ).toEqual([]);
    expect(
      newWarnings,
      `Console warnings after menu toggle:\n${newWarnings.join("\n")}`,
    ).toEqual([]);
  });
  ...
  test("language switcher round-trip bokmål ↔ nynorsk produces no console errors", async ({
    ...
    test.skip(
      browserName === "webkit",
      "AxiosError: Network Error on WebKit during language switch — application bug",
    );
    const checker = createConsoleChecker(page, CONSOLE_WHITELIST);
    await page.goto("/");
    ...
    const isMobile =
      (await desktopLink.isVisible().catch(() => false)) === false;
    if (isMobile) {
  ...
  test("homepage shows both school levels", async ({ page }) => {
    ...
    await expect(
      page
        .locator("h3")
        .filter({ hasText: /mellomtrinn/i })
        .first(),
    ).toBeVisible();
    await expect(
      page
        .locator("h3")
        .filter({ hasText: /ungdom/i })
        .first(),
    ).toBeVisible();
  });
  ...
  test("school-level switch navigates between levels and produces no console errors", async ({
    ...
    expect(
      newErrors,
      `Console errors after school-level switch:\n${newErrors.join("\n")}`,
    ).toEqual([]);
    expect(
      newWarnings,
      `Console warnings after school-level switch:\n${newWarnings.join("\n")}`,
    ).toEqual([]);
  });
  ...
  test("in-page level switch is interactive (.btn-switch-container)", async ({
    ...
    test.skip(
      !containerVisible,
      ".btn-switch-container not present on this page",
    );
  ...
  test("cookie consent banner is present and dismissible", async ({ page }) => {
    ...
    await expect(
      banner,
      "Cookie consent banner should be visible on first visit",
    ).toBeVisible();
  ...
  test("footer navigation links are present and reachable", async ({
    page,
  }) => {
    await page.goto("/");
    await dismissCookieBanner(page);
